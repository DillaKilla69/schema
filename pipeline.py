...
from czml_builder import build_czml
    ...
    try:
        conn.execute(
            ...
            INSERT INTO run_history (
                ...
                dry_run, used_cache, estimated_row_count, artifact_dir, czml_path
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                result.run_id,
...
def run_pipeline(
    ...
    """
    Validate selections, generate SQL artifacts, optionally execute and build CZML.
    ...
    if not dry_run:
        ...
        czml_packets = build_czml(rows, document_name=f"Run {run_id}")
        czml_file = run_dir / "output.czml.json"
        _write_json(czml_file, czml_packets)
        czml_path = str(czml_file.resolve())
